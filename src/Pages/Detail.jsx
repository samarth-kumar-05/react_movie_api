...
const Detail = () => {
  ...
  return (
    ...
    >
      ...
      {/* booking model  */}
      {show && (
        <form className="absolute top-0 bg-white/95 w-full h-full grid place-items-center">
          <div
          ...
          <section className="max-w-[40rem] mx-auto border border-para_text rounded-lg p-5 ">
            ...
            <div className="flex gap-4 max-w-[20rem] flex-wrap justify-center mx-auto py-10">
              {box.map((item) => (
                <span
                  key={item}
                  className="w-[2rem] h-[2rem] bg-blue hover:bg-para_text border border-white rounded-md cursor-pointer"
                />
              ))}
            </div>
            <Link to="/">
